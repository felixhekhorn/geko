...
*                                                                 *
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
c 2025-01-22/FH: modified to use f2py. To compile run `python -m numpy.f2py -c -m grvphoton grvphoton.f`
C
       SUBROUTINE GRVGLO (X, Q2, UL, DL, SL, CL, BL, GL)
cf2py intent(in) X, Q2
cf2py intent(out) UL, DL, SL, CL, BL, GL
c       IMPLICIT real (A - Z)
...
c       write(6,*) 'Ubar'
c       write(6,16) S,AK,AG,BG,C,BK,AL,E,ES,BE,D
...
c       write(6,*) 'sbar'
c       write(6,16) S,SF, AL, BE, AK, BK, AG, BG, C, D, E, ES
...
cf2py intent(in) X, Q2
cf2py intent(out) UH, DH, SH, CH, BH, GH
c       IMPLICIT real (A - Z)
       IMPLICIT DOUBLE PRECISION (A-Z)
       MU2  = 0.3
       LAM2 = 0.248 * 0.248
...
C      renamed function F to FONE
       double precision FUNCTION FONE(X,S,AL,BE,AK,BK,AG,BG,C,D,E,ES)
c       IMPLICIT real (A - Z)
       IMPLICIT DOUBLE PRECISION (A-Z)
       SX = SQRT (X)
       LX = dLOG (1./X)
     ...
     1    * EXP (-E + SQRT (ES * S**BE * LX))) * (1.- X)**D
       ...
       END
C
       double precision FUNCTION FS(X,S,SF,AL,BE,AK,BK,AG,BG,C,D,E,ES)
c       IMPLICIT real (A - Z)
       IMPLICIT DOUBLE PRECISION (A-Z)
       IF (S .LE. SF) THEN
          FS = 0.0
